...
levels(df$Scientific_name)

df$Variance<-(df$Error_new)^2
df$tsquare<-df$Rel_assay_temp^2
df$tcubic<-df$Rel_assay_temp^3


###### metafor model, used to obtain some values and matrix structure for I² calculations
full_meta2 <- rma.mv(Sel_coeff, Variance, random = list(~ 1 | Study_ID / Curve_ID, ~ 1 | Scientific_name),
                     data = df, method="ML",
                     mods = ~ Type_Gen_var+Sign+Rel_assay_temp+ tsquare + tcubic )
summary(full_meta2)


##### obtain model matrix
W<- diag(1/full_meta2$vi)
X <- model.matrix(full_meta2)
...
post0<-read.table(file="Multi_posteriors_JEB.txt", header= TRUE,sep = "\t")
...
##### t_sigma = Species
##### b_sigma = Curve
sum_posts2 <- post0$s_sigma^2 + post0$t_sigma^2 + post0$b_sigma^2




# get I² total
mean(100 * sum_posts2 / (sum_posts2 + (full_meta2$k-full_meta2$p)/sum(diag(P))))

# get I² for the s_sigma component
mean(100 * post0$s_sigma^2 / (sum_posts2 + (full_meta2$k-full_meta2$p)/sum(diag(P))))

# get I² for the t_sigma component
mean(100 * post0$t_sigma^2 / (sum_posts2 + (full_meta2$k-full_meta2$p)/sum(diag(P))))

# get I² for the b_sigma component
mean(100 * post0$b_sigma^2 / (sum_posts2 + (full_meta2$k-full_meta2$p)/sum(diag(P))))




# get I² CIs total 
quantile(100 * sum_posts2 / (sum_posts2 + (full_meta2$k-full_meta2$p)/sum(diag(P))),p=c(0.025,0.5,0.975))

# get I² CIs for the s_sigma component
quantile(100 * post0$s_sigma^2 / (sum_posts2 + (full_meta2$k-full_meta2$p)/sum(diag(P))),p=c(0.025,0.5,0.975))

# get I² CIs for the t_sigma component
quantile(100 * post0$t_sigma^2 / (sum_posts2 + (full_meta2$k-full_meta2$p)/sum(diag(P))),p=c(0.025,0.5,0.975))

# get I² CIs for the t_sigma component
quantile(100 * post0$b_sigma^2 / (sum_posts2 + (full_meta2$k-full_meta2$p)/sum(diag(P))),p=c(0.025,0.5,0.975))
