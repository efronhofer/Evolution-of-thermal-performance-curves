...
full_meta <- rma.mv(Sel_coeff, Variance, random = list(~ 1 | Study_ID / Curve_ID, ~ 1 | Scientific_name),
                    data = df, method="ML" )
summary(full_meta)
...
post0<-read.table(file="posteriors_2_JEB.txt", header= TRUE,sep = "\t")
...
mean(100 * post0$b_sigma^2 / (sum_posts2 + (full_meta$k-full_meta$p)/sum(diag(P))))
...
# get I² CIs for the t_sigma component
quantile(100 * post0$b_sigma^2 / (sum_posts2 + (full_meta$k-full_meta$p)/sum(diag(P))),p=c(0.025,0.5,0.975))
